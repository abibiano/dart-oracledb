...
created: '2026-05-21'
status: 'done'
status_reason: 'All code changes shipped (version gates, env normalization, docker-compose/CI for oracle21c). The blocking FAST_AUTH-against-pre-23 issue (Known Issue #2) was resolved by spec-classical-auth-fallback.md (status: done), which added the transport.supportsFastAuth-gated classical AUTH_PHASE_ONE/TWO path. Oracle 21c integration suite now authenticates and passes.'
...
- **2026-05-22 (marked done):** `spec-classical-auth-fallback.md` is `done`. `AuthFlow.authenticate` now branches on `transport.supportsFastAuth` — Oracle 21c takes the classical AUTH_PHASE_ONE/TWO path and authenticates successfully. All original spec tasks are complete. Spec closed as `done`.
