...
- Native platform support declared for macOS (Apple Silicon and Intel), Windows, Linux, Android, and iOS; CI currently verifies macOS, Windows, and Linux
...
| Platform Support | Works on native Dart targets: macOS Apple Silicon/Intel, Windows, Linux, Android, iOS; web excluded | `pubspec.yaml` platform declaration plus desktop/server CI matrix |
...
|--------|-------------|
| **Language** | Dart |
| **Minimum SDK** | Dart 3.12+ |
| **Platforms** | macOS (Apple Silicon, Intel), Windows, Linux, Android, iOS |
| **Compilation** | JIT (development), AOT (production) |
| **Mobile/Web** | Android and iOS supported as native Dart targets; web not supported because it lacks `dart:io` TCP sockets |
...
- `pointycastle` - Authentication crypto, including SHA-512 hashing, PBKDF2 key derivation, HMAC, and AES operations
...
- NFR11: Package must support Dart SDK 3.12+
- NFR12: Package must work on native Dart platforms: macOS (Apple Silicon and Intel), Windows, Linux, Android, and iOS; web is explicitly unsupported
...
Added 2026-06-13 via the Correct Course workflow
(`planning-artifacts/sprint-change-proposal-2026-06-13.md`). These requirements
extend the shipped 1.0.0 MVP toward node-oracledb thin-mode parity. They are
**not** part of the original 1.0 MVP scope; full epic/story decomposition lives
in `planning-artifacts/epics.md` (Epics 8–15) and tracking in
`implementation-artifacts/sprint-status.yaml`.

| Epic | Capability | Notes |
|------|------------|-------|
| 8 | Streaming & incremental result consumption (`OracleResultSet`, `queryStream`) | Architecture review required |
| 9 | REF CURSOR & implicit results | Depends on Epic 8 |
| 10 | Non-AL32UTF8 database character set compatibility | Architecture review required |
| 11 | Bulk DML (`executeMany`) | — |
| 12 | LOB streaming & temporary LOBs | Extends Epic 4 |
| 13 | JSON / OSON parity | Extends Story 4.4 |
| 14 | Temporal compatibility & fetch type handlers | TSTZ region names, fetch-as-string |
| 15 | Extended data type completeness | INTERVAL, ROWID/UROWID, VECTOR |

**Explicitly out of scope (pure-Dart thin driver):** AQ/TxEventQ (deferred until
concrete user need); SODA, CQN/subscriptions, external/Kerberos auth, and native
network encryption (all thick-mode in the reference, incompatible with the
no-Oracle-Client direction).

All post-1.0 stories inherit the mandatory dual-environment validation rule
(Oracle 23ai **and** 21c) per `project-context.md` §Testing Rules.
